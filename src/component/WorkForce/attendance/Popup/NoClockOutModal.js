    ...
    <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center p-4 z-50">
      <div className="bg-white rounded-lg shadow-xl w-full max-w-7xl max-h-[80vh] flex flex-col">
        <div className="flex justify-between items-center border-b border-gray-200 px-6 py-4">
          <h2 className="text-xl font-semibold text-gray-900">No Clock-Out Employees - {formatRange(timeFilter)}</h2>
        ...
        <div className="flex-1 overflow-auto p-6">
          {loading ? (
            <div className="text-center py-8">
              ...
              <p className="text-gray-500 mt-2">Loading no clock-out employees...</p>
            </div>
          ) : error ? (
            ...
            <div className="text-center py-8 text-gray-500">
              <p>No employees found without clock-out.</p>
            </div>
          ) : (
              ...
              <table className="min-w-full divide-y divide-gray-200">
                <thead className="bg-gray-50">
                  <tr>
                    ...
                    <th className="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">Total No Clock-Out Days</th>
                ...
                <tbody className="bg-white divide-y divide-gray-200">
                  {Object.values(groupedEmployees).map((employeeGroup) => (
                    <React.Fragment key={employeeGroup.employee_id}>
                      <tr className={`${expandedEmployees[employeeGroup.employee_id] ? 'bg-red-50' : ''} cursor-pointer`}>
                        <td className="px-6 whitespace-nowrap">
                          <button
                            onClick={() => toggleExpand(employeeGroup.employee_id)}
                            className="text-sm p-1 bg-red-200 rounded-lg"
                          >
        ...
        <div className="border-t border-gray-200 px-6 py-4">
          <div className="flex justify-between items-center">
            <span className="text-sm text-gray-500">
              Showing {Object.keys(groupedEmployees).length} employees with {employees.length} total no clock-out records
